...
def generate_ad_set(
    ...
    if config.get("expand_image", False) and image:
        expansion_response = expand_image(
            api_key=api_key,
            image_data=image,
            prompt=config.get("expansion_prompt", "natural background"),
            expansion_direction=config.get("expansion_direction", "all"),
            expansion_ratio=config.get("expansion_ratio", 1.5),
            num_results=config.get("num_results", 1)
        )
        result["expanded"] = expansion_response
